"""Equity Baron: adaptive pot-odds and value-pressure bot."""
...
def texture(board):
    ...
    wet = (0.34 if connected else 0) + (0.34 if flushy else 0) + (0.12 if paired else 0)
...
def foldiness(state):
    ...
    for p in state["players"]:
        ...
        if st and st["actions"] >= 5:
            rates.append(st["folds"] / max(1, st["actions"]))
    return sum(rates) / len(rates) if rates else 0.33
...
def risk_numbers(state, pos):
    ...
    stack_ratio = state["your_stack"] / max(1, avg)
    phase = clamp(hand_num(state) / 400.0, 0, 1)
    mom = sum(MOMENTUM) / START
    agg = 0.38 + pos * 0.13 + phase * 0.10 + clamp(stack_ratio - 1, -1, 2) * 0.13
    agg += foldiness(state) * 0.12 + clamp(mom, -1, 1) * 0.08
    fe = 0.36 + foldiness(state) * 0.22 + pos * 0.11 + agg * 0.10
    ...
    return clamp(agg, 0.15, 0.82), clamp(fe, 0.05, 0.78)
...
def size(state, kind, agg, wet):
    ...
    if kind == "value":
        mult = random.choice([0.55, 0.70, 0.88])
    elif kind == "thin":
        mult = random.choice([0.35, 0.45, 0.55])
    elif kind == "semi":
        mult = random.choice([0.50, 0.66, 0.82])
    else:
        mult = random.choice([0.42, 0.58, 0.72])
...
def preflop(state, pos, agg, fe):
    ...
    pot = max(1, state["pot"])
    score = s + pos * 0.12 + fe * 0.15 + agg * 0.10
    stack_bb = state["your_stack"] / BB

    if stack_bb <= 10 and score > 0.82:
        return {"action": "all_in"}
    if owed == 0:
        if score > 0.62:
            return {"action": "raise", "amount": legal_raise(state, state["min_raise_to"] * random.choice([2.2, 2.7, 3.2]))}
        return {"action": "check"}
    if s > 0.80:
        return {"action": "raise", "amount": legal_raise(state, max(state["min_raise_to"], state["current_bet"] * random.choice([2.5, 3.2, 4.0])))}
    price = owed / max(1, pot + owed)
    if score > 0.75 and owed < state["your_stack"] * 0.24:
        return {"action": "raise", "amount": legal_raise(state, state["current_bet"] + pot * random.choice([0.75, 1.0]))}
    if price < s * 0.38 + 0.06:
        return {"action": "call"}
...
def postflop(state, pos, agg, fe):
    ...
    eq = equity(hole, board, opps, 900 if opps == 1 else 560)
    draw = draw_score(hole, board)
    owed = state["amount_owed"]
    pot = max(1, state["pot"])
    price = owed / max(1, pot + owed)
    bluff = fe * 0.42 + agg * 0.24 + pos * 0.12 + draw - wet * 0.12 - max(0, opps - 1) * 0.08
    ...
    if state["can_check"]:
        ...
        if cat >= 2 or eq > 0.62:
            return {"action": "raise", "amount": size(state, "thin", agg, wet)} if random.random() < 0.65 else {"action": "check"}
        if draw and bluff > 0.45:
            return {"action": "raise", "amount": size(state, "semi", agg, wet)}
        if opps <= 2 and bluff > 0.58 and random.random() < bluff * 0.55:
    ...
    if cat >= 5 or eq > 0.82:
        if agg + fe > 0.95:
            return {"action": "raise", "amount": size(state, "value", agg, wet)}
        return {"action": "call"}
    if cat >= 2 and eq > price + 0.08:
        return {"action": "call"}
    if draw and eq + draw > price + 0.04 and owed < state["your_stack"] * 0.32:
        if bluff > 0.62 and random.random() < 0.45:
            return {"action": "raise", "amount": size(state, "semi", agg, wet)}
        return {"action": "call"}
    if bluff > 0.70 and opps == 1 and owed < state["your_stack"] * 0.20 and random.random() < 0.35:
        return {"action": "raise", "amount": size(state, "bluff", agg, wet)}
    if eq > price + 0.07 and not (owed > pot * 0.70 and cat == 0):
        return {"action": "call"}
    return {"action": "fold"}
